-- V37: Move V9/V26/V27 changes into a new migration
-- Bu script, daha önce değiştirdiğin V9, V26 ve V27 migration'larındaki
-- yeni mantığı güvenli şekilde uygular.

-----------------------------------------------------------------------
-- BÖLÜM 1 - V9'dan gelen index / constraint / trigger iyileştirmeleri
-- (Tamamı idempotent olacak şekilde yazılmıştır)
-----------------------------------------------------------------------

-- Add additional performance indexes (idempotent)
...
-- Re-define check constraints in an idempotent way
...
-- Unique constraints - make them consistent and stricter where desired
...
-- Burada partial unique yerine tam UNIQUE'e geçiyoruz
...
-- Drop & recreate updated_at triggers idempotently
...
CREATE TRIGGER trigger_breed_updated_at
...
CREATE TRIGGER trigger_medical_history_updated_at
    BEFORE UPDATE ON medical_history
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_clinical_examination_updated_at
    BEFORE UPDATE ON clinical_examination
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trigger_radiological_imaging_updated_at
    BEFORE UPDATE ON radiological_imaging
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_medicine_updated_at
    BEFORE UPDATE ON medicine
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_prescription_updated_at
...
CREATE TRIGGER trigger_vaccination_record_updated_at
    BEFORE UPDATE ON vaccination_record
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_pathology_findings_updated_at
    BEFORE UPDATE ON pathology_findings
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trigger_invoice_item_updated_at
...
CREATE TRIGGER trigger_stock_product_updated_at
    BEFORE UPDATE ON stock_product
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_stock_transaction_updated_at
    BEFORE UPDATE ON stock_transaction
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER trigger_reminder_updated_at
    BEFORE UPDATE ON reminder
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_report_schedule_updated_at
    BEFORE UPDATE ON report_schedule
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();


-----------------------------------------------------------------------
-- BÖLÜM 2 - V26'dan gelen created_by=NULL iyileştirmeleri
-----------------------------------------------------------------------

-- Boncuk için koşulların created_by alanını NULL'a çek
UPDATE animal_conditions ac
SET created_by = NULL
FROM animal a
JOIN owner o ON a.owner_id = o.owner_id
WHERE ac.animal_id = a.animal_id
  AND a.name = 'Boncuk'
  AND o.email = 'mehmet.demir@demo.com'
  AND ac.name = 'Tavuk Proteini';
...
WHERE ac.animal_id = a.animal_id
  AND a.name = 'Boncuk'
  AND o.email = 'mehmet.demir@demo.com'
  AND ac.name = 'Böbrek Yetmezliği (Evre 1)';

-- Rex için koşulların created_by alanını NULL'a çek
UPDATE animal_conditions ac
SET created_by = NULL
FROM animal a
JOIN owner o ON a.owner_id = o.owner_id
...
WHERE ac.animal_id = a.animal_id
  AND a.name = 'Rex'
  AND o.email = 'mehmet.demir@demo.com'
  AND ac.name = 'Kalça Displazisi';


-----------------------------------------------------------------------
-- BÖLÜM 3 - V27'den gelen created_by=NULL iyileştirmeleri
-----------------------------------------------------------------------

-- Demo Admin sahibinde created_by alanını NULL yap
UPDATE owner
SET created_by = NULL
WHERE email = 'demo.admin@hss.com';

-- Pamuk hayvanında created_by alanını NULL yap
UPDATE animal a
SET created_by = NULL
FROM owner o
WHERE a.owner_id = o.owner_id
  AND a.name = 'Pamuk'
  AND o.email = 'demo.admin@hss.com';

-- Pamuk'un koşullarında created_by alanını NULL yap
UPDATE animal_conditions ac
SET created_by = NULL
FROM animal a
JOIN owner o ON a.owner_id = o.owner_id
